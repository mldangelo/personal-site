...
The app's main job, for me, is keeping parallel work apart. I can see what each agent is doing in its own thread, its changes stay walled off in a worktree, and the diff and screenshots are right there when I want to check the result. A "task," then, is really a bundle: a thread, its worktree, the diff, and whatever I ran to verify it. That bundling is what lets me keep several going without losing the thread of any one.

I used to approximate this with terminal panes, naming conventions, and notes to myself. The cost was holding it all in my head: which pane was which, which branch went with which task, what I'd already verified, what I still needed to say. The specific UI — tabs or not — is beside the point. What I needed was for that state to live somewhere other than my memory.

Queued follow-ups are a small thing I lean on more than I expected. If I spot a correction while a thread is still working, I queue the instruction and keep moving, so I can steer an agent without dropping what I'm doing.
...
Worktrees were already central to my Claude Code setup; they let me run several tasks against one repo without them stepping on each other. What Codex changed is that spinning up a worktree and pinning it to a thread got easy enough that I started scoping tasks around them.

Before I hand something off, I ask whether it makes a good worktree. A good one has a clear goal, a diff I'll be able to review, a concrete way to confirm it worked, and a way to back out if it didn't. If I can't describe those last three up front, the task usually isn't ready. The other thing I like is that a worktree is disposable: an agent can try an approach, I can look at what it did, and if it went sideways I throw it away without touching anything else.
...
Browser and computer use are the most useful of these features and the ones I'm most careful with. I split them across three surfaces: the in-app browser for local previews, file-backed pages, and public pages; the Chrome extension for signed-in work where Codex needs my actual browser profile; and supervised computer use for GUI tasks outside the browser. Keep local QA in the in-app browser. Signed-in sites need tighter approvals and allowlists, because that's a live session you're handing over. Desktop access is the one I watch hardest, since it drives the real app with real permissions. (At launch, computer use was macOS-only.)

The case I lean on most is background QA. One thread makes a change in a worktree while another opens the local preview, walks the changed path, and reports back with screenshots and whatever broke. I get a second pass without pausing the implementation, and what comes back is concrete: a screenshot, the step that failed, and a diff to review. Outside QA, computer use is good for the awkward jobs that are too tedious to do by hand but too small to build a tool for: bulk cleanup in a bad web UI, a fiddly testing flow, pulling signal out of an interface that fights you.

Security is the real catch here. The moment an agent can read and act on content, that content becomes part of your attack surface. The realistic threat is prompt injection through ordinary material: a PDF, a spreadsheet, a PR description, a browser tab, or an email thread can each carry instructions to an agent that holds tools. A signed-in page is effectively your account, and desktop access widens the blast radius further. It's worth the risk under supervision, with tight scope and a human in the loop, which is exactly why the sandboxing, approval, and network work is what makes these usable for real development instead of one-off demos.

## Limits and rough edges

Some of this was just operational. Over April, the Claude Code tradeoffs got more visible to me: quota behavior, the third-party harness policy, tokenizer changes, a couple of incidents. Anthropic raised Claude Code limits on May 6, which is good for users.

Codex has its own rough edges. High subagent counts burn tokens fast, background work can spray out into noisy checks, and the browser and computer-use features need careful permissions. The app has more surface area than a terminal-first tool, so I spend more time deciding which surface should own a given task. And once agents were running all day, a pile of unglamorous things — quota and cache behavior, permissions, branch isolation, whether a thread resumes cleanly — became what decides whether I can work for hours without babysitting. I pay far more attention now to how a tool exposes its state and where it lets me review than I did a few months ago.

Token efficiency is why GPT-5.5 is relevant here. OpenAI says it uses significantly fewer tokens on the same Codex tasks, and the docs recommend it for most work when it's available. I haven't benchmarked that myself, but at this kind of volume, token efficiency turns straight into lower cost and fewer limit walls.
