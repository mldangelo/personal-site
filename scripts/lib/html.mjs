...
export function tags(html, name = '[a-z][\\w:-]*') {
  // `name` is a pattern, and callers pass alternations such as `img|source`.
  // Without the group, `\b[^>]*>` binds to the last branch only, so `<img …>`
  // is matched by `<img` alone and every other branch silently loses its
  // terminator.
