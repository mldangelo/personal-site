...
/**
 * The layout's `<head>`, rendered the way Next's own renderer would.
 *
 * `next/script` reads `appDir` off `HeadManagerContext` and only that branch
 * emits the App Router's deferred `(self.__next_s=…).push(…)` queue. A bare
 * `renderToStaticMarkup` supplies no provider, so `appDir` is falsy, a
 * `beforeInteractive` `<Script>` renders to *nothing at all*, and every
 * assertion about how the bootstrap was queued passes against the very
 * regression it guards — measured: the `__next_s` check below held green
 * against a `<Script strategy="beforeInteractive">` layout. Providing the
 * context is what makes it reachable.
 */
  ...
  /<head>([\s\S]*?)<\/head>/.exec(
    renderToStaticMarkup(
      createElement(
        ...
        { value: { appDir: true } },
        createElement(rootLayout.default, { children: null }),
      ),
    ),
  )?.[1] ?? '';
