...
- **Career chronology is derived, never hand-maintained**: `src/lib/career.ts` owns the resume timeline. `sortPositions()` orders roles by recency of involvement (end date first); an open-ended `commitment: 'part-time'` role is placed by its start so a long-running side engagement does not outrank later primary jobs, and `tierFor()` never promotes that side role to the lead visual tier. Source order in `src/data/resume/work.ts` is not load-bearing. `positionDuration()` derives each tenure, while `careerSpanYears()` is only the elapsed span since the earliest role—not summed active experience—so public copy must call it a career span. The page reads `Date.now()` once and threads it down; `Job` requires that shared instant. Visible duration abbreviations have an unabbreviated `.sr-only` label. Ordering compares validated `YYYY-MM-DD` strings directly rather than parsing to `Date`
