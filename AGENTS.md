...
- **Print**: `app/styles/print.css` is imported last so it can override both themes. The resume is the page people print; after changing its content or layout, build the export, print the actual `/resume/` route to both Letter and A4 PDFs, render every page to images, and inspect the pages plus PDF link annotations. Repeat with a saved dark theme: Chromium applies `color-scheme` to the physical page canvas outside `html`/`body`, so white element backgrounds alone can still leave dark margins. Browser width is not a proxy for paper geometry either, and the surprise runs the other way: at `@page { margin: 16mm 14mm }` the print viewport measures ~710px on Letter and ~690px on A4, so **screen `max-width` breakpoints fire on paper**. The 735px breakpoint silently collapsed the two-column course list to thirteen stacked rows in every PDF until `print.css` re-declared the grid. Measure the print viewport with a `@media print and (min-width: N)` probe rather than reasoning from the paper size. Let a wrapping contact line use flex gaps between unbreakable items rather than punctuation separators, which can dangle at either edge of a line
