...
- **Icons**: `app/favicon.ico`, `app/icon.png`, `app/apple-icon.png`, and `app/manifest.json` are Next's **static file** conventions, and all four survive `output: 'export'` with real extensions — verified in `out/`, which is why they are safe where `app/opengraph-image.tsx` is not. Two traps. A `manifest.ts` **route** is a different thing: it emits `/manifest.webmanifest` and fails the build outright without `export const dynamic = 'force-static'`, so the static `manifest.json` is preferred here — `.json` also has a content type GitHub Pages certainly knows. And declaring `metadata.icons` _replaces_ the convention-derived links rather than adding to them, exactly like `openGraph`. Nothing is `<link>`ed by hand. Regenerate with `npm run icons` (`scripts/generate-icons.mjs`) and commit the art with `scripts/icons.meta.json`; `scripts/__tests__/generate-icons.test.ts` fails when the art falls behind `--color-accent`, the exact checksummed display-font input, or the Next renderer version. `public/images/favicon/` used to hold 29 files and 275 KiB of pre-redesign art that was directly addressable but neither linked nor conventionally discovered. Files under `public/` are ordinary assets and always need an explicit link or manifest reference; only files under `app/` participate in Next's metadata conventions, and only `favicon.ico` is restricted to the root app segment
- **Colours outside CSS**: `theme-color` and the icon art need literal hex, which no custom property gives them. Read it at build with `readColorToken` in `src/lib/tokens.ts` rather than typing it — the old icon set was still carrying `#2e59ba` long after the accent became `#1b2fbf`, because a hex typed into a file has nothing checking it. `theme-color` must come from `--color-bg-alt`, the page background the sticky header tints, not `--color-bg`. With JavaScript, a script-owned unscoped tag follows the resolved `data-theme` and survives client navigation; the `<noscript>` fallback is light because that is the only theme the CSS serves without the bootstrap
- **Testing what `next/script` emits**: `renderToStaticMarkup` supplies no `HeadManagerContext`, and `next/script` reads `appDir` from it to decide whether to emit the App Router form — the deferred `(self.__next_s=self.__next_s||[]).push(...)` queue. With no provider, `appDir` is falsy, a `beforeInteractive` `<Script>` renders to **nothing at all**, and any assertion about how a script was queued passes against every implementation including the broken one. Render inside `HeadManagerContext.Provider value={{ appDir: true }}` (from `next/dist/shared/lib/head-manager-context.shared-runtime`), as `app/__tests__/theme-color.test.ts` does, so the guard that the theme bootstrap is a literal parser-blocking `<script>` rather than a queued one can actually fail. Do not reach for `data-nscript` as the marker: it is only ever set client-side by `el.setAttribute`, so it never appears in server markup at all
