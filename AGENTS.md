...
- **Competency needs an explicit equivalent**: the old skill tags varied size and weight by their 1–5 self-score, while a numeric `aria-label` on a generic `<span>` was ignored. The current UI reduces that score to `core` / `working` / `familiar`, explains the visual weights once in a legend, and repeats each tag's tier in `.sr-only` text. Keep all three in sync. The former per-category `color` field was unrelated to competency and produced the same decorative tick for every category
