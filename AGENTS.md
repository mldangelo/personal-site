...
- **Focus rings are two tokens**: `--color-focus-ring` for controls on the page, `--color-focus-ring-fill` for accent-filled ones, where the accent ring is the fill's own colour. Every explicit `outline` uses one of them; `src/lib/__tests__/contrast.test.ts` fails on a palette colour
- **Contrast checks are a floor, not a proof**: grain, `backdrop-filter`, and blending sit between the two flat values a token pair describes, so a green run still needs eyeballing on the rendered page. Assert thresholds in tests and keep the measured ratios in the token comments, or the same number drifts in two places
- **Forced colors**: `app/styles/forced-colors.css` overrides whichever theme is active, unlayered, before `print.css`. Only `border-color` and `outline-color` are remapped for you, so anything drawn with `background` needs an explicit rule — restate higher-specificity variants (`.jobs-container:last-child::before`, `.hamburger-button:focus-visible …`) or the element disappears in that one state. `Highlight` is only guaranteed against `HighlightText`: use the pair for selection, `CanvasText` for marks on the page
...
- **Numbers about the site**: Never type a figure describing the codebase into a data file. `/stats` claimed 2,272 lines against ~4,400 actual before it was switched to `countSourceLines()` in `src/lib/loc.ts`. Anything countable should be counted at build. It counts every non-test `.ts`/`.tsx` under `app/` and `src/`, so a test-only helper belongs in its test file — vitest treats everything under `__tests__/` as a suite, so it cannot live there as a module
