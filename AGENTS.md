...
- **Page transitions need a real trigger**: the removed `app/styles/utilities.css` opted cross-document navigations into the View Transition API with `@view-transition` and styled the root transition pseudo-elements, but normal internal navigation goes through Next `<Link>` and the app has no same-document trigger such as `document.startViewTransition()` or React `<ViewTransition>`. If transitions return, choose and test the navigation path that starts them and add explicit reduced-motion behavior; `experimental.viewTransition` is one possible Next integration, not a requirement of the web API. Do not leave comment-only placeholder stylesheets behind either — `components/forms.css` sat in the import graph and shipped nothing
