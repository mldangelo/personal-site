...
- **Telemetry**: Measured values shown on `/stats` live in `src/lib/telemetry.ts`. Live readings use `useLiveAge`, which paints a fixed-width digit-free placeholder until the first client tick — test that path with `renderToStaticMarkup`, since effects flush during Testing Library's `render()`
...
- **Motion**: Anything that animates continuously must check `prefers-reduced-motion` (see `useLiveAge`, `useScrollAnimation`, `EmailLink`). The live age readout takes a single reading under the preference rather than ticking
