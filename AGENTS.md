...
- **Export size budget**: `npm run measure-export` weighs `out/` by subsystem and gates it against `scripts/budget.json`; CI runs it after `npm run verify-export` on every build leg. Bundle analysis only sees the JavaScript graph, and most of what Pages serves here is not JavaScript — RSC prefetch payloads are ~25% of the export and images another ~24%. The budgets are ratchets carrying documented headroom, not byte fingerprints: a limit pinned to the current byte count turns every honest pull request red. Raise one on purpose with `npm run measure-export -- --update-budget` and say why in the PR. Deliberately has no page in the site — writing a manifest into the export changes the export's own size, so the on-page version is a fixed point that needs a second build pass on every matrix leg. JSON reports must stay outside `out/`, and an export with assets but no HTML fails closed rather than reporting the per-route gates at zero. **No gated metric may be a function of the page count** — the inline-SVG gate first shipped reading `repeatedBytes`, which is arithmetically the icon set times the number of documents, so a third blog post turned it red while every other budget sat near 83%. It gates `distinctBytes` (the icon set itself, invariant under page count) and prints the repeated total ungated. Apply that test to any metric added here: simulate a few extra posts against a real `out/` before committing the budget
