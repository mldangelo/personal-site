...
- **Career chronology is derived, never hand-maintained**: `src/lib/career.ts` owns the resume timeline. `sortPositions()` orders the spine (newest start first, later end as tiebreak), so source order in `src/data/resume/work.ts` is not load-bearing and a new role can be appended anywhere — the array had silently drifted to 2022 → 2017 → 2014 → 2015 → 2014 while `Experience` rendered it raw. `positionDuration()` and `totalExperienceYears()` derive every tenure and the "N+ years" span in `app/resume/page.tsx`, which was previously typed prose. Report exactly one experience figure: the elapsed span since the first role. Showing it beside a months-actually-occupied figure reads as hedging, not precision. These functions take `now` as an argument and never read the clock, the same contract as `ageAt()` in `src/lib/telemetry.ts`; the page reads `Date.now()` once and threads it down. Ordering compares the ISO strings directly rather than parsing to `Date`, so `src/data/__tests__/work.test.ts` pins the `YYYY-MM-DD` format the comparison depends on
...
- **Print**: `app/styles/print.css` is imported last so it can override both themes. The resume is the page people print; check it there after changing resume layout. Printed output is verifiable rather than guessable: copy the built CSS out of `out/_next/static/css/`, replace `@media print{` with `@media screen{`, and read `getComputedStyle(el, '::after')` in headless Chrome (`--headless --dump-dom` on a page that writes results into the DOM). That is how the href-reveal rule below was checked instead of argued about
- **Print reveals hrefs, which makes resume URLs user-visible text**: the `attr(href)` rule in `print.css` covers only the _identifying_ link of an entry — `.job-company`, `.degree-container .school a`, `.course-container a` — deliberately not citations inside a role summary (three URLs in one sentence of 10pt serif) and not `.resume-print-contact`, whose link text is already the readable address. Three things to know before touching it: `.course-container a` is `display: grid`, so its `::after` becomes a third grid item and lands in the 5.5rem code column unless placed with `grid-column`; the claim that the global `a { background-image: none !important }` further down overrides `a[href^='http']::after { content: … }` is false, because the reset declares no `content` and `background-image` does not inherit into a pseudo-element (measured, not assumed); and since `[href^='http']` is the selector, a relative or missing URL in `src/data/resume/*.ts` prints with no destination at all. Long URLs are a data problem, not a `word-break` problem — the CME 302 link was a 131-character SCPD URL carrying a `jsessionid`, session-scoped and now a 403. `app/__tests__/print-resume.test.tsx` pins the selector coverage, the exclusions, the grid placement, and rejects session ids or anything over 80 characters
