...
- **Page transitions**: `@view-transition` only fires on cross-document navigation, which Next `<Link>` bypasses, so a site-wide cross-fade declared that way never runs
