...
- **Canonical/export URLs**: When generating absolute URLs for metadata, RSS, sitemap, or schema, match `trailingSlash: true` output (`/about/`, `/writing/post-slug/`) instead of non-canonical no-slash variants; file-like routes such as `/feed.xml` and `/sitemap.xml` stay file-like. `trailingSlash: true` is build-time config that the unit-test environment never applies, so `next/link` renders `/writing/slug` under jsdom while the export writes `/writing/slug/`. Assert canonical shape against `out/`, and normalise the slash away in component tests rather than pinning whichever shape currently passes
- **Publishing a post changes what the newest-item tests see**: a draft becoming published moves the newest dated item, which drives the featured card on `/writing` and `lastBuildDate` in the feed. Tests must derive those from `getWritingItems()`, never pin a literal date or href — two did, and merging the publication of a post turned `main` red for correct behaviour. Note that a post-only branch can pass its own suite while failing after merge, so run the suite on the merge, not just the branch
- **Source-backed post claims need stable semantics**: replace rolling phrases such as “last month” with the actual observation window when publication can move, and link consequential claims where the reader encounters them. Token efficiency is not the same as cost: do not turn “fewer tokens” into “lower spend” without measuring the pricing or quota system, especially when the cited source says the model costs more per token
...
- **Renaming post images is a `git rm`, not a copy**: nothing in CI flags an unreferenced file under `public/`, so a rename done by copying ships both bytes to the export forever. Both deletion directions are gated after the fact — dropping the referenced copy reddens `verify-export` with "image points at missing export", dropping the orphan reddens `src/lib/__tests__/imageSize.test.ts` — so the real cost is dead weight plus a fixture anchored to a file no page uses. Point that fixture at an image the site actually renders
