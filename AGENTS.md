...
- **Print reveals hrefs, which makes resume URLs user-visible text**: the `attr(href)` rule in `print.css` covers only the _identifying_ link of a professional or education entry — `.job-company` and `.degree-container .school a` — deliberately not citations inside a role summary (three URLs in one sentence of 10pt serif), not `.resume-print-contact` (its link text is already the readable address), and not course links. Chrome preserves anchors as clickable PDF annotations, while printing all 13 raw catalogue URLs made the paper version dense and broke hostnames across lines. The claim that the global `a { background-image: none !important }` further down overrides `a[href^='http']::after { content: … }` is false, because the reset declares no `content` and `background-image` does not inherit into a pseudo-element (measured, not assumed). Long URLs remain a data problem — the CME 302 link was a 131-character SCPD URL carrying a `jsessionid`, session-scoped and now a 403. `app/__tests__/print-resume.test.tsx` pins the selector coverage, exclusions, screen-chrome suppression, and link-data invariants
