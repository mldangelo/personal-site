...
- **Career chronology is derived**: `src/lib/career.ts` orders the resume spine by end date rather than start, placing an open-ended `commitment: 'part-time'` role by its start so a side engagement does not outrank later primary jobs. Anything naming the job held now — the footer, the JSON-LD `Person` node — must call `currentPosition()`, never `work[0]`, because a job change recorded the natural way leaves the new role last in the array
