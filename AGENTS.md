...
- **Motion**: Anything that animates continuously must check `prefers-reduced-motion` through the shared `usePrefersReducedMotion` hook (see `useLiveAge`, `EmailLink`) rather than hand-rolling a `matchMedia` read. A value styled or labelled as live must actually be updating; the age restores its dated server snapshot when motion is reduced or the page is hidden
