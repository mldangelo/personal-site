...
- **Theming**: two attributes on `<html>`, both stamped before first paint by the inline bootstrap in `app/layout.tsx`. `data-theme` (`light`/`dark`) is the resolved theme every stylesheet keys off. `data-theme-choice` (`light`/`dark`/`system`) is what the visitor asked for, and exists because `data-theme` cannot tell a forced dark theme from a followed one — without it `ThemeToggle` could not know its own state before mounting, so it reserved a blank 44x44 hole in the header until React hydrated. Keep the bootstrap as a literal head `<script>`: App Router serializes `next/script` with `beforeInteractive` into a `self.__next_s` descriptor that the client bootstrap executes later, which is too late for a first-paint theme decision. The bootstrap and the control share their attribute, storage, and media-query vocabulary through `src/lib/theme.ts`; a test executes the generated script and guards its necessarily duplicated storage parsing against drift. CSS hides the theme and mobile-menu controls until `data-theme-choice` exists: if the bootstrap did not run, their handlers cannot hydrate and the page must not expose enabled buttons that promise otherwise. Write that gate as `:root:not([data-theme-choice]) .x { display: none }`, never as a base `display: none` plus a `:root[data-theme-choice]` restore — the restore ties on specificity with `.hamburger-button { display: flex }` in `header.css`, so the gate would be decided by `@import` order alone, and its `(0,3,0)` weight silently outranked the `@media screen and (min-width: 736px)` desktop hide in the same file, leaving that rule inert. `system` is stored as the _absence_ of the `theme` key, so a visitor who never chose and one who chose to follow their device are the same state. Persist through `window.localStorage`, never bare `localStorage`, to keep Node runtime globals out of browser-only paths. The pre-paint bootstrap is an inline script in a static export, not intrinsically CSP-safe; a future strict `script-src` policy would need to allow its exact hash (or permit inline scripts)
